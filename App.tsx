...
const App: React.FC = () => {
  ...
  // (Assuming typical placement, but I will place it before handleLoadProject or after)

  const handleSaveToMatter = () => {
    // Send results to parent window (Chamber.Law)
    const message = {
      type: 'CHAMBER_SAVE_EXTRACTION',
      payload: {
        results,
        documents: documents.map(d => ({ id: d.id, name: d.name })),
        columns: columns.map(c => ({ id: c.id, name: c.name, prompt: c.prompt }))
      }
    };

    window.parent.postMessage(message, '*');
    // We could wait for a response, but for now simple fire-and-forget UI feedback
    alert("Sent data to Chamber.Law for saving...");
  };
  ...
  // Check for URL params to load document (Deep Integration)
  React.useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const docUrl = params.get('docUrl');
    const docName = params.get('docName') || 'Imported Document';

    // Safety check to prevent infinite re-loading or duplicate loading
    // We check if a document with this name already exists in state
    if (docUrl && !documents.some(d => d.name === docName)) {
      const loadExternalDoc = async () => {
        setIsConverting(true);
        try {
          // Fetch the blob from the SAS URL
          ...
          // Clean up URL to prevent reload loops if we were using pushState (optional)
          // window.history.replaceState({}, document.title, window.location.pathname);
        } catch (err) {
          console.error("Failed to load external doc:", err);
          // alert("Failed to load document from Chamber.Law. Link may be expired.");
        } finally {
          setIsConverting(false);
        }
      };
                  ...
                  <div className="absolute right-0 top-full mt-2 w-56 bg-white rounded-xl shadow-xl border border-slate-100 p-1 z-50 animate-in fade-in zoom-in-95 duration-100">
                    {MODELS.map(model => (
                      <button
                        ...
                        className={`w-full text-left px-3 py-2 rounded-lg flex items-center gap-3 transition-colors ${selectedModel === model.id ? 'bg-indigo-50 text-indigo-700' : 'hover:bg-slate-50 text-slate-700'
                          }`}
                      >
